...
data = pd.read_csv('consommation-annuelle-residentielle-par-adresse.csv',sep = ';',index_col='Année',parse_dates=True)
...
model=ARIMA(train["Consommation annuelle totale de l'adresse (MWh)"],select =(1,0,3))
model=model.fit()
model.summary()
start=len(train)
end=start+96
